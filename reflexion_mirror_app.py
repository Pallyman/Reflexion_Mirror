...
def export_reflection(format, reflection_id):
    ...
    if format == 'txt':
        ...
        # Markdown export
        # Preprocess the narrative so that newline escapes do not appear inside
...
#
# The application originally contained a block of narrative generation logic
# directly inline after the /api/reflexion-dna route.  Because it was placed
# after a return statement it was never executed, and the variables it
# referenced (collapse, build, now, archetype) were not defined in that
# context.  This resulted in unreachable code and latent NameErrors.  To
# restore the intended functionality, we encapsulate that logic into a
# dedicated function.  This helper builds a multi‑section narrative for a
# user's transformation journey based on their inputs and chosen archetype.
...
def generate_narrative(collapse: str, build: str, now: str, archetype: str) -> str:
    ...
    Constructs a structured, multi‑section narrative from the collapse,
    build and now phases of the user's journey, weaving in the selected
    archetype.  Each section is separated by headings and explanatory
    sentences to guide the reader through the transformation.
...
def send_reflection_email(email, narrative, reflection_id):
    ...
    # Compose the plain‑text body using a properly quoted f‑string
...
"""
    ...
    # Compose the HTML body using a triple‑quoted f‑string.  The HTML is indented
    # inside the string for readability but will be rendered without the leading
    # whitespace.
    msg.html = f"""
    <html>
